...
Regexp: class {
    ...
    */
    compile: static func ~withOptions(pattern: String, options: Int) -> This {
        p := Pcre compile(pattern data, options, (Regexp errorMsg&) as const Char**, Regexp errorOffset&, null)
        if(!p) {
    ...
    */
    matches: func ~withLengthAndStart(subject: String, start: Int, length: SizeT) -> Match {
        ...
        count := pcre exec(null, subject data, length, start, 0, ovector, maxSubstrings)
        if(count > 0) {
...
Match: class extends Iterable<String> {
    ...
    */
    group: func ~byName(name: String) -> String {
        number := regexp pcre getStringNumber(name data)
