                    ...
                    if(!(expr instanceOf?(BoolLiteral) && expr as BoolLiteral getValue() == true)) {
                        ...
                        if(caseExpr instanceOf?(VariableDecl)) {
                                ...
                                if(res fatal) {
                                    res throwError(
                                        CantUseMatch new(expr token,
                                            "You can't use the type match syntax here, can't resolve `%s`" cformat(fCall toString() toCString())
                                    ))
                                } else {
                            ...
                            if(fCall getRef() != null) {
                                ...
                                if(returnType != "Bool")
                                    res throwError(WrongMatchesSignature new(expr token, "matches? returns a %s, but it should return a Bool" cformat(returnType toCString())))
                                caze setExpr(fCall)
                            } else {
